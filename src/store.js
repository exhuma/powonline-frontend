      ...
      */
      addTeam (state, team) {
        ...
        for (let [idx, entry] of state.teams.entries()) {
          LOG.debug(`New team has effectiveStartTime ${team.effective_start_time} entry-time: ${entry.effective_start_time}`)
        ...
        if (foundPosition) {
          LOG.debug(`Inserting ${team} at position ${insertPosition}`)
          state.teams.splice(insertPosition, 0, team)
        } else {
          LOG.debug(`Appending ${team} at end`)
          state.teams.push(team)
        }
        LOG.debug('teams is now:')
        LOG.debug(state.teams)
      },
      ...
      addStation (state, station) {
        ...
        for (let [idx, entry] of state.stations.entries()) {
          if (entry.order >= station.order) {
            LOG.debug(`New station has order ${station.order} which goes before ${entry.name} with order ${entry.order}`)
            foundPosition = true
            insertPosition = idx
            break
          }
        ...
        if (foundPosition) {
          LOG.debug(`Inserting ${station} at position ${insertPosition}`)
          state.stations.splice(insertPosition, 0, station)
        } else {
          LOG.debug(`Appending ${station} at end`)
          state.stations.push(station)
        }
        LOG.debug('Stations is now:')
        LOG.debug(state.stations)
      },
      ...
      */
      assignTeamToRouteRemote (context, data) {
        ...
        remoteProxy.addTeamToRoute(data.routeName, team)
          ...
          .catch(e => {
            window.console.error({msg: 'Unable to assign team to route', exc: e})
            EventBus.$emit('activityEvent', {
              visible: false,
              progress: -1,
              text: ''
      ...
      */
      unassignTeamFromRouteRemote (context, data) {
        ...
        remoteProxy.unassignTeamFromRoute(data.routeName, data.teamName)
          ...
          .catch(e => {
            window.console.error({msg: 'Unable to unassign team from route', exc: e})
            EventBus.$emit('activityEvent', {
              visible: false,
              progress: -1,
              text: ''
      ...
      */
      assignStationToRouteRemote (context, data) {
        ...
        remoteProxy.assignStationToRoute(data.routeName, station)
          ...
          .catch(e => {
            window.console.error({msg: 'Unable to assign station to route', exc: e})
            EventBus.$emit('activityEvent', {
              visible: false,
              progress: -1,
              text: ''
      ...
      */
      unassignStationFromRouteRemote (context, data) {
        ...
        remoteProxy.unassignStationFromRoute(data.routeName, data.stationName)
          ...
          .catch(e => {
            window.console.error({msg: 'Unable to unassign station from route', exc: e})
            EventBus.$emit('activityEvent', {
              visible: false,
              progress: -1,
              text: ''
