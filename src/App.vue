<template>
  <div id="app">
    <v-app dark>
      ...
      >
        {{ globalSnackText }}
        <v-btn flat @click="globalSnack = false">Close</v-btn></v-snackbar
      >
      <v-slide-y-transition>
        <v-toolbar v-if="isTitleBarVisible" app>
          ...
          <v-tooltip bottom v-if="tokenIsAvailable">
            <v-btn slot="activator" @click.native.stop="logoutUser" icon
              ><v-icon>exit_to_app</v-icon></v-btn
            >
            <span>Logout</span>
          </v-tooltip>
          <v-tooltip bottom v-else>
            <v-btn slot="activator" @click.native.stop="showLoginDialog" icon
              ><v-icon>perm_identity</v-icon></v-btn
            >
            <span>Login</span>
          </v-tooltip>
        </v-toolbar>
      ...
      >
        <v-list>
          <v-list-tile v-for="route in routes" :to="route.to" :key="route.to">
            <v-list-tile-action
              ><v-icon>{{ route.icon }}</v-icon></v-list-tile-action
            >
            <v-list-tile-content>
            ...
            <v-card>
              ...
              <v-card-text>
                <v-tabs v-model="activeLoginTab" grow>
                  <v-tabs-bar>
                    <v-tabs-item href="#socialLogin" key="socialLogin" ripple
                      >Social Login</v-tabs-item
                    >
                    <v-tabs-item href="#localLogin" key="localLogin" ripple
                      >Local Login</v-tabs-item
                    >
                    <v-tabs-slider color="accent" />
                  </v-tabs-bar>
                  <v-tabs-items>
                    <v-tabs-content key="socialLogin" id="socialLogin">
                      <v-btn class="mt-5 mb-5" @click="login('google')"
                        >Google</v-btn
                      >
                      <v-btn class="mt-5 mb-5" @click="login('facebook')"
                        >Facebook</v-btn
                      >
                      <v-card-actions>
                        <v-spacer />
